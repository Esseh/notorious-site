...
var global_image_input = document.querySelector("#js-cropper-input");
var global_rotate_btn = document.querySelector("#js-crop-rotate");
var global_zoom_range = document.querySelector("#js-crop-zoom");
var global_cropper_controls = document.querySelector(".js-crop-controls");
var global_filename = document.querySelector('.js-cropper-filename');
var cropX = document.querySelector('#js-posx');
var cropY = document.querySelector('#js-posy');
var cropWidth = document.querySelector('#js-cropwidth');
var cropHeight = document.querySelector('#js-cropheight');
var degrees = document.querySelector("#js-degrees");
var global_cropper_submit_label = document.querySelector(".js-cropper-submit-label");
var cropper_checkbox = document.querySelector("#cropper-modal-checkbox");

canvas = document.getElementById('js-cropper-canvas');
global_image_input.addEventListener('change', initCropper);

function initCropper() {
  var file = global_image_input.files[0];
  var ctx = canvas.getContext('2d');
  resetImageCropper(cropper);
  if(!file) {
    return;
  }
  ...
  image.onload = function() {
    var originalRatio = Math.min(image.width / image.height, image.height / image.width);
    var img_limit = canvas.parentNode.clientWidth; //canvas width img_limit
    var img_original = {width: image.width, height: image.height}; //save image original size

    global_filename.innerHTML = file.name;
    if(originalRatio <= 1 && image.width >= image.height) {
      image.width = img_limit;
      image.height = image.width * originalRatio;
    } else if(image.height >= img_limit) {
      image.height = img_limit;
      image.width = image.height * originalRatio;
    }
    ...
    cropper = new Cropper(canvas, {
      ...
      zoomOnWheel: false,
      ready: function() {
        global_cropper_controls.style.display = "block";
        global_rotate_btn.addEventListener("click", rotateImageCropper.bind(cropper));
        cropper.zoomTo(1);
        global_zoom_range.addEventListener("change", function(){cropper.zoomTo(this.value)}); //IE fix
        global_zoom_range.addEventListener("input", function(){cropper.zoomTo(this.value)});

        var cropsvg = document.createElementNS("http://www.w3.org/2000/svg","svg");
        cropsvg.innerHTML = "<use xlink:href='#svg-tictactoe'></use>";
        document.querySelector(".cropper-view-box").appendChild(cropsvg);
      },
      crop: function(e) {
        var cropper_canvas = document.querySelector(".cropper-canvas");
        var cropper_crop_box = document.querySelector(".cropper-crop-box");
        var width = image.width;
        var height = image.height;
        if(degrees.value == 90 || degrees.value == 270) {
          width = image.height;
          height = image.width
        }
        var rx = img_original.width/image.width;
        var ry = img_original.height/image.height;
        cropX.value = Math.round(e.detail.x*rx);
        cropY.value = Math.round(e.detail.y*ry);
        cropWidth.value = Math.round(e.detail.width*rx);
        cropHeight.value = Math.round(e.detail.height*ry);
        degrees.value = e.detail.rotate;
      }
    });
  }
  image.src = imagePath;
...
function cropper_close(cropper, checkbox) {
  if(!checkbox.checked) resetImageCropper(cropper);
  if(checkbox.checked) global_image_input.value = "";
}

function resetImageCropper(cropper) {
  if(cropper) {
    cropper.destroy();
  }
  canvas.width = 300;
  ...
  global_zoom_range.value = 1;
  //global_cropper_controls.style.display = global_cropper_submit_label.style.display = "none";
  //global_filename.innerHTML = "No File Chosen...";
}

function rotateImageCropper() {
  var d = global_rotate_btn.getAttribute("value");
  d += 90;
  if(d == 360) d = 0;
  this.rotate(d);
}
